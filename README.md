...
VectorizedKmers.jl is a Julia package for fast $k$-mer counting of biological sequences. The core idea is that $k$-mers with an alphabet size of $A$ are essentially integers in base $A$, and can be used as indices in a vector of size $S^k$ to count the corresponding $k$-mers.
