...
## Limitations
The main downside of counting k-mers this way is that, unless [SparseArrays.jl](https://github.com/JuliaSparse/SparseArrays.jl) is used, the vectors will grow immensely large as $k$ increases. The 31-mer count vector of a single DNA sequence would have a length of $4^{31} = 4,611,686,018,427,387,904$, which is equivalent to four exabytes of memory, if the counts are stored as 8-bit integers. This method of counting K-mers therefore works best for lower k-values in the 5 to 9 range, unless [SparseArrays.jl](https://github.com/JuliaSparse/SparseArrays.jl) are used.